...
const SCENARIOS = [
  'Breaking Bad News (Terminal Illness)',
  'Ethics Discussion (Refusing life-saving treatment)',
  'Professional Communication (Inter-department conflict)',
  'Medical Negligence accusation handling',
  'Consent for high-risk surgery'
];
...
export default function AETCOMSkillsPage() {
  ...
  const handleStartSession = () => {
    ...
    const promptText = `Doctor, please tell me the truth. The test results came back, didn't they? I need to know what we are dealing with regarding the ${activeCase}. Please, don't hide anything.`;
  ...
  const stopRecordingAndAnalyze = async () => {
    ...
    try {
      const res = await fetch('/api/grade-proskill', {
        ...
        body: JSON.stringify({ transcript: userTranscript, caseType: activeCase, type: 'aetcom' })
      });
    ...
    <div className="page-container" style={{ paddingBottom: 60 }}>
      <div className="page-header" style={{ marginBottom: 30 }}>
        <h1 className="page-title font-outfit" style={{ fontSize: 28, color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 28 }}>ðŸ¤</span> AETCOM Skills
        </h1>
        <p className="page-desc">Attitude, Ethics, and Communication Simulators (Breaking Bad News, Ethics Discussions)</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: sessionStarted ? '1fr' : '1fr', gap: 24, maxWidth: 1000, margin: '0 auto' }}>
        ...
        {!sessionStarted ? (
          <div className="card shadow-sm animate-fadeIn" style={{ borderTop: '4px solid var(--primary)' }}>
            <h2 style={{ fontSize: 18, marginBottom: 16 }}>Configure AETCOM Scenario</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 20 }}>
              ...
              <div className="form-group" style={{ margin: 0 }}>
                ...
                <select className="input-field" value={caseType} onChange={e => setCaseType(e.target.value)}>
                  ...
                  <option value="Custom">Other (Custom AETCOM Case)</option>
                </select>
              </div>
            </div>
            ...
            {caseType === 'Custom' && (
              <div className="form-group animate-fadeIn" style={{ marginBottom: 20 }}>
                <label className="label">Enter Custom AETCOM Case / Condition</label>
                <input className="input-field" value={customCase} onChange={e => setCustomCase(e.target.value)} placeholder="e.g. Dealing with an angry patient's relative" />
              </div>
            )}
            ...
            <div style={{ textAlign: 'center', marginTop: 30, padding: 30, background: '#F8FAFC', borderRadius: 8, border: '2px dashed #CBD5E1' }}>
              ...
              <p style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 20, maxWidth: 500, margin: '0 auto 20px' }}>
                Ensure your microphone is connected and speakers are turned on. The AI will vocally present an ethical dilemma or tough situation, and you will need to respond professionally.
              </p>
              <button 
          ...
          <div className="animate-fadeIn">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 20 }}>
              <div className="card shadow-sm" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 20px', minHeight: 400, position: 'relative', overflow: 'hidden' }}>
                ...
                {sessionStage === 'ai_speaking' && (
                  <div className="animate-fadeIn" style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--primary)', marginBottom: 16 }}>Simulated Person is speaking...</div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, marginBottom: 30, width: '100%', maxWidth: 500 }}>
                      ...
                      <div style={{ width: '100%', textAlign: 'left' }}>
                        ...
                        <textarea 
                          ...
                          placeholder="Speak your professional response clearly..."
                ...
                {sessionStage === 'analysing' && (
                  <div className="animate-fadeIn" style={{ textAlign: 'center' }}>
                    ...
                    <h3 style={{ fontSize: 18, color: 'var(--primary)' }}>Analysing Professional Ethics with AI...</h3>
                    <p style={{ color: 'var(--text-muted)' }}>Evaluating SPIKES protocol adherence, empathy, and ethical frameworks based on your transcript.</p>
                  </div>
                )}
                ...
                {sessionStage === 'result' && evaluation && (
                  <div className="animate-fadeIn" style={{ width: '100%' }}>
                    <div style={{ textAlign: 'center', marginBottom: 24 }}>
                      ...
                      <h3 style={{ fontSize: 22, color: 'var(--text)' }}>AETCOM Evaluation</h3>
              ...
              <div className="card shadow-sm" style={{ background: '#F8FAFC' }}>
                <h3 style={{ fontSize: 16, marginBottom: 12, borderBottom: '1px solid #E2E8F0', paddingBottom: 8 }}>AETCOM Grading Criteria</h3>
                <ul style={{ fontSize: 13, color: '#334155', paddingLeft: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <li><strong>SPIKES Protocol:</strong> Setting, Perception, Invitation, Knowledge, Empathy, Summary.</li>
                  <li><strong>Non-verbal cues:</strong> Addressed via tone and pacing.</li>
                  <li><strong>Ethical Boundaries:</strong> Upholding autonomy, beneficence, and justice.</li>
                  <li><strong>Conflict Resolution:</strong> Ability to de-escalate effectively.</li>
                </ul>
              </div>
            </div>
          </div>
