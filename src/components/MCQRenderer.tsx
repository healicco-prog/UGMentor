...
function parseMCQText(text: string) {
  ...
  return blocks.map(block => {
    ...
    const questionMatch = block.match(/^(.*?)(?=a\))/si);
    let question = questionMatch ? questionMatch[1].trim() : block;
    question = question.replace(/^\d+\.\s+/, '');

    // Options
    const optA = block.match(/a\)\s*(.*?)(?=b\))/si)?.[1]?.trim() || '';
    const optB = block.match(/b\)\s*(.*?)(?=c\))/si)?.[1]?.trim() || '';
    const optC = block.match(/c\)\s*(.*?)(?=d\))/si)?.[1]?.trim() || '';
    const optD = block.match(/d\)\s*(.*?)(?=Answer:|$)/si)?.[1]?.trim() || '';

    // Answer and Reason
    const answerMatch = block.match(/Answer:\s*([a-d])\)?\s*(.*?)(?=Reason:|$)/si);
    const correctLetter = answerMatch?.[1]?.toLowerCase() || '';
    const answerText = answerMatch?.[2]?.trim() || '';

    const reasonMatch = block.match(/Reason:\s*(.*)/si);
