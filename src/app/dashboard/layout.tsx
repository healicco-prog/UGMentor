...
interface NavItem {
  ...
  icon: string;
  href?: string;
  module?: string; // for access control
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!isLoading && user?.role === 'superadmin') router.replace('/contrl-panl');
  }, [user, isLoading, router]);

  // Auto-expand the parent group when navigating to a child route,
  // but don't force it — user can still click to collapse afterwards.
  useEffect(() => {
              ...
              <div key={item.id} className={styles.navGroup}>
                {item.children ? (
                  <button
                    ...
                    onClick={() => accessible ? toggleExpand(item.id) : null}
