...
export default function StatisticsAssistantPage() {
  const [topic, setTopic] = useState('');
  const [details, setDetails] = useState('');
  const [taskType, setTaskType] = useState('Sample Size Calculator');
  ...
  return (
    <div className="page-container" style={{ padding: 'clamp(16px, 3vw, 32px)', maxWidth: '1200px', margin: '0 auto' }}>
      <div className="page-header" style={{ marginBottom: 32, textAlign: 'center' }}>
        <h1 className="page-title font-outfit" style={{ fontSize: 'clamp(24px, 5vw, 32px)', color: 'var(--primary)', fontWeight: 800 }}>📈 Statistics Assistant</h1>
        <p className="page-desc" style={{ fontSize: 'clamp(14px, 2vw, 16px)', color: '#64748B' }}>Calculate sample sizes, select the right statistical tests, and interpret results.</p>
      </div>
      ...
      <div className="card" style={{ marginBottom: 32, width: '100%', padding: 'clamp(20px, 4vw, 32px)', borderRadius: 16, boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.05)' }}>
        <div className="form-group" style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            {['Sample Size Calculator', 'Statistical Test Selection', 'Interpretation Assistant'].map(type => (
        ...
        <div className="form-group" style={{ marginBottom: 20 }}>
          <label className="label">Research Topic / Objective</label>
          <input 
            className="input-field" 
            placeholder="e.g., Comparing blood pressure between two diet groups" 
            value={topic} 
        ...
        <div className="form-group" style={{ marginBottom: 24 }}>
          <label className="label">Details & Variables (Optional)</label>
          <textarea 
            className="input-field" 
            placeholder="List your variables (e.g., Age, Gender, BP in mmHg)..." 
            value={details} 
